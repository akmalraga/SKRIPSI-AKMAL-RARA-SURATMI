...
\begin{figure}[h]
  \centering 
  \includegraphics[width=0.7\linewidth]{picture/graphene.png}
  \caption{Visualisasi struktur kristal grafena. Setiap titik mewakili posisi atom karbon.}
  \label{fig:vis_graphene}
\end{figure}
...
\subsection{Representasi Bloch}
Mengikuti formulasi yang dijabarkan oleh \parencite{Yusufaly2013} \parencite{Cole_Python_Tight_Binding_2025},
\par 
Vektor dalam kisi didefinisikan sebagai
\[
 R = n_1 a_1 + n_2 a_2 + n_3 a_3 
\]
dengan melakukan transformasi kedalam ruang reciprocal, kita peroleh vektor
\[
\begin{align}
...
\]
\par
Misalkan $\mu$ adalah label, sehingga $t_\mu$ adalah lokasi dari atom pada unit kisi. Untuk basis sistem Tight-Binding, seperti yang disinggung pada \ref{subsec:tight-binding}, dapat dituliskan sebagai $\zeta_{\mu\alpha}(r-R-t_mu)$. Dengan memperkanalkan index $j=\mu\alpha$
\[
\phi_{R_j} (r) = \phi_j (r - R) = \zeta_{\mu \alpha}(r - R - t_{\mu})
\]
\[
\langle \phi_{R_j}| \phi_{R_j} \rangle = \delta_{RR'}\delta_{ij}
\]
dan hamiltonian
\[
H_{ij}(R) = \langle \phi_{R'_i}|H| \phi_{R' + R_j} \rangle = \langle \phi_{0_i}|H| \phi_{R_j} \rangle 
\]
\par 
Untuk konvensi Bloch, paket PythTb \parencite{Cole_Python_Tight_Binding_2025} \parencite{Coh2022} menggunakan dua konvensi Bloch. Namun dalam kasus topologi yang memerlukan informasi probabilitas elektron pada setiap unit cell, kita akan terfokus pada konvensi dengan penambahan fase extra.
\par
Dengan mengkonstruksi fungsi \textit{Bloch-like}
\[
\begin{align}
\mathcal{X}^k_j(r) = \sum_R e^{i k \cdot (R + t_j)} \phi_j(r - R) \\
|\mathcal{X}^k_j \rangle = \sum_R e^{i k \cdot (R + t_j)} |\phi_{R_j}\rangle \\
\langle \mathcal{X}|\mathcal{X'} \rangle = \int_{cell} d^3 r \mathcal{X}'(r) \\
\langle\mathcal{X}^k_j |\mathcal{X}^k_j \rangle = \delta_{ij}
\end{align}
\]
yang kemudian di ekspansikan sepanjang nilai $j$ untuk memperoleh fungsi Bloch
\[
|\psi_{nk} \rangle = \sum_j \mathcal{C}_j^{nk} |\mathcal{X}_j^k \rangle
\]
Matriks Hamiltonian kemudian dapat diperoleh sebagai
\[
H_{ij}^k = \langle \mathcal{X}|H |\mathcal{X'} \rangle = \sum_R e^{i k \cdot (R + t_j -t_i)}H_{ij}(R)
\]
dimana solusi dari persamaan secular dapat diselesaikan secara langsung lewat Diagonalisasi untuk memperoleh nilai dan fungsi eigen 
\[ 
\mathcal{H}_k \cdot \mathcal{C}_{nk} = E_{nk}\mathcal{C}_{nk}
\]
Karena penelitian ini mengkaji model dengan pertimbangan parameter spin, paket yang dikembangkan oleh \parencite{Cole_Python_Tight_Binding_2025} juga menyediakan solusi diagonalisasi dengan formulasi matematis
\[ 
\begin{align}
n_{i, ss'} = \frac{1}{N} \sum_{nk}^{occ}(\mathcal{C}^{nk}_{is})^* \mathcal{C}^{nk}_{is'} \\
H_{ij, ss'} = \sum_a h_{ij,a} \sigma_{a, ss'}
\end{align}
\]
\subsection{Validasi Efek Tepi}

Setelah memperoleh spektrum energi dari diagonalisasi hamiltonian pada ruang diskrit, kita akan memperoleh spektrum energi dari sistem yang periodik, dimana elektron memiliki potensi untuk menjelajah seluruh titik kisi dengan probabilitas universal. Namun dalam kasus konduktansi tepi, elektron tidak menjelajah seluruh titik kisi, melainkan termodulasi ketepian. Maka untuk memvalidasi model dan prosedur numerik, kita perlu menghitung spektrum energi sistem yang terbatas(tidak periodik). Hal ini dilakukan dengan \textit{slab geometry}, dimana ruang tiga dimensi direduksi menjadi dimensi yang lebih rendah \parencite{Cole_Python_Tight_Binding_2025}.
\par
Untuk keperluan validasi, penelitian ini menggunakan arah (100), dimana arah y dan z tetap periodik sedangkan arah x terbatas. Secara konsep, seharusnya struktur pita akan menunjukkan terciptanya celah pita pada energi fermi akibat terlokalisasinya elektron di \textit{bulk}, namun akan ada dua pita yang berbelok dari valensi ke pita konduktif, \textit{vice verca} \parencite{PhysRevB.91.085106} .

\par
Pada penelitian ini, efek dari \textit{Periodic Boundary Condition} dan \textit{Open Boundary Condition} juga diuji, untuk melihat bagaimana distribusi energi yang diperbolehkan pada sistem ketika diterapkan batas pada material dan bagaimana respon sistem ketika diberi kondisi periodik \parencite{Cole_Python_Tight_Binding_2025} .
...
\par
Untuk menentukan kriteria dari \textit{edge state} kita hanya perlu melihat pencampuran probabilitas pada struktur pita. Pada insulator trivial, probabilitas elektron akan kontras antara valensi dan konduksi, berbeda dengan mode tepi yang akan mencampur probabilitas \parencite{PhysRevB.111.205139}
...
\par
Rapat keadaan dalam mekanika kuantum dimodelkan sebagai \parencite{GROSSO20141} ;
\[
...
\begin{figure}[h]
  \centering 
  \includegraphics[width=1.0\linewidth]{picture/Z2_Winding.png}
  \caption{Perbedaan pola \textit{winding} dari fase (A) \textit{Quantum Spin Hall} dan (B) Insulator \citep{PhysRevB.84.075119}}
\label{fig:Z2_winding}
\end{figure}
...
Dengan demikian, prosedur Soluyanov-Vanderbilt untuk menentukan invarian $Z_2$ dilakukan dengan memplot evolusi fase $\theta_m$ terhadap momentum tegak lurusnya ($k_\perp$). Dalam fase topologis ($Z_2=1$), spektrum fase ini akan menunjukkan fenomena \textit{partner switching}, di mana jalur-jalur pusat muatan Wannier saling bertukar pasangan Kramers \ref{fig:Z2_winding}. Secara praktis, algoritma ini memastikan bahwa perhitungan tetap \textit{gauge-invariant}, sehingga sifat topologi material dapat diidentifikasi secara stabil tanpa memerlukan syarat \textit{gauge-fixing} yang rumit.

\subsection{\textit{Axion Angle}}
Gerakan orbital elektron dalam kristal tiga dimensi menciptakan \textit{Pseudoscalar magnetoelectric phase coupling} $\theta$ \parencite{PhysRevLett.102.146805}. Parameter $\theta$ berawal dari upaya konstruksi bentuk \textit{covariant} dari teori medan elektromagnetik \parencite{O'Dell01031963}, dimana bentuk Lagrangian Maxwell dituliskan sebagai.
\[
\Delta \mathcal{L}=\kappa a \mathbf{E \cdot B}
\]
namun dalam upaya perhitungan numerik, \parencite{O'Dell01031963} menunjukkan bahwa bentuk ini perlu didefinisikan ulang sebagai
\[
\begin{align}
\Delta \mathcal{L} = (\theta e^2 / 4 \pi^2)trF_{\mu \nu}\tilde{F}_{\mu \nu} \\
= (\theta e^2 / 2 \pi^2)(\mathbf{E \cdot B} + \dots)
\end{align}
\]
Di dalam medium, Lagarangian Maxwell terkoneksi langsung dengan polarisasi dan magnetisasi.
\[
  \mathbf{P} = \frac{\partial \mathcal{L}}{\partial \mathbf{E}} \qquad
  \mathbf{M} = \frac{\partial \mathcal{L}}{\partial \mathbf{B}}
\]
\par 
Dengan meninjau hubungan tensor polarisasi dan magnetisasi dan hubungannya dengan tensor medan elektromagnetk, polarisabilitas magnetoelectric akhirnya dapat dirumuskan dengan menurunkan polarisasi terhadap medan magnet dan atau turunan magnetisasi terhadap medan listrik\parencite{PhysRevLett.58.1799}.
\[
\alpha_{ij} = \frac{\partial M_{j}}{\partial E_{i}}|_{\mathbf{B}=0} = \frac{\partial P_{i}}{\partial B_{j}}|_{\mathbf{E}=0 }
\]
dimana tensor medan elektromagnetk dan polarisasi magnetisasi dapat dituliskan sebagai
\[
F_{\mu \nu} = \begin{pmatrix}
0 & -E_{x} & -E_{y} & -E_{z} \\ 
E_{x} & 0 & -cB_{z} & cB_{y} \\
...
\end{pmatrix}
\]
dari fakta bahwa tensor polarisabilitas ini terdiri dari komponen diagonal dan non-diagonal, maka $\alpha_{ij}$ dapat dibagi menjadi dua bagian; bagian diagonal yang disebut \textit{axion coupling} $\alpha^{\text{axion}}$ dan bagian non-diagonal yang disebut \textit{traceless coupling} $\alpha^{\text{traceless}}$ \parencite{PhysRevLett.102.146805}.
\[
\alpha_{ij} = \tilde{\alpha}_{ij} + \frac{\theta e^2}{2 \pi h}\delta_{ij}
\]
\par
Fitur esensial dari $\theta$ adalah bahwa nilai konstan menunjukkan bahwa $\theta$ tidak mempengaruhi persamaan Maxwell, namun perubahan nilai $\theta$ akan mempengaruhi persamaan Maxwell. Fitur inilah kemudian yang akan kita memanfaatkan untuk mendeteksi perubahan sifat topologi pada sistem. ketika kita melakukan perubahan perlahan pada parameter sistem, kita akan melihat bagaimana perubahan nilai $\theta$ mempengaruhi sifat magnetoelectric sistem, yang mana perubahan ini akan menunjukkan perubahan sifat topologi pada sistem.
\par
...
Untuk membawa persamaan ini ke dalam perhitungan numerik, \parencite{PhysRevLett.102.146805} mempertimbangkan masalah \textit{gauge-fixing} yang dapat mempengaruhi akurasi perhitungan, seperti yang disinggung pada \ref{subsec:HWF}. Untuk itu dengan melakukan modifikasi yang meninjau polarisasi pada penelitian \parencite{Vanderbilt2018}, \parencite{PhysRevLett.102.146805} melakukan prosedur transisi dengan memvariasikan parameter dari $\mathbf{k}$ menuju $\beta$ secara perlahan, sehingga $\theta$ dapat dihitung sebagai integrasi terhadap $\beta$. $\theta(\beta)$ dihitung untuk ekspresi \textit{second chern number} menggunakan \textit{Four Curvature Formulation}
\[
