...
\begin{align}
    ...
    \gamma_n(t) &= i \int_0^t ds \quad \dot{\vec{R}}(s)\langle n, \vec{R}(s)| \nabla_{\vec{R}} |n, \vec{R}(s) \rangle \\ 
                &= i \int_{\vec{R_1}}^{\vec{R_2}} d\vec{R} \; \dot{\vec{R}}(s)\langle n, \vec{R}| \nabla_{\vec{R}} |n, \vec{R} \rangle
\end{align}

\section{Halangan Topologi pada Fungsi Wannier}

Definisi Fungsi Wannier $| W_{n\mathbf{R}} \rangle$ dari transformasi Fourier fungsi Bloch $|\psi_{n\mathbf{k}}\rangle$:
\begin{align}
    | W_{n\mathbf{R}} \rangle = \frac{V}{(2\pi)^d} \int_{\text{BZ}} d\mathbf{k} \, e^{-i\mathbf{k}\cdot\mathbf{R}} |\psi_{n\mathbf{k}}\rangle
\end{align}
Fungsi Bloch memiliki kebebasan \textit{gauge} terhadap pergeseran fase $\theta(\mathbf{k})$:
\begin{align}
    |\tilde{\psi}_{n\mathbf{k}}\rangle = e^{i\theta(\mathbf{k})} |\psi_{n\mathbf{k}}\rangle
\end{align}
Syarat lokalisasi eksponensial $| W_{n\mathbf{R}} \rangle$ mengharuskan $|\tilde{\psi}_{n\mathbf{k}}\rangle$ bersifat kontinu dan analitik (\textit{smooth}) di seluruh Zona Brillouin (BZ).
\\~\\
Koneksi Berry $\mathbf{A}(\mathbf{k})$ dan Kelengkungan Berry $\Omega(\mathbf{k})$ didefinisikan dari bagian periodik fungsi Bloch $|u_{n\mathbf{k}}\rangle$:
\begin{align}
    \mathbf{A}(\mathbf{k}) &= i \langle u_{n\mathbf{k}} | \nabla_{\mathbf{k}} | u_{n\mathbf{k}} \rangle \\
    \mathbf{A}'(\mathbf{k}) &= \mathbf{A}(\mathbf{k}) - \nabla_{\mathbf{k}} \theta(\mathbf{k}) \\
    \Omega(\mathbf{k}) &= \nabla_{\mathbf{k}} \times \mathbf{A}(\mathbf{k})
\end{align}

Berdasarkan Teorema Stokes, integrasi Kelengkungan Berry pada seluruh permukaan tertutup BZ ($T^2$) adalah:
\begin{align}
    \int_{\text{BZ}} \Omega(\mathbf{k}) \, d^2k &= \int_{\text{BZ}} \left[ \nabla_{\mathbf{k}} \times \mathbf{A}(\mathbf{k}) \right] d^2k \nonumber \\ 
    &= \oint_{\partial(\text{BZ})} \mathbf{A}(\mathbf{k}) \cdot d\mathbf{l}
\end{align}
Jika $\mathbf{A}(\mathbf{k})$ bersifat \textit{smooth} dan kontinu secara global di seluruh BZ, maka integral kontur pada batas zona akan saling meniadakan:
\begin{align}
    \oint_{\partial(\text{BZ})} \mathbf{A}(\mathbf{k}) \cdot d\mathbf{l} = 0 \label{eq:stokes_zero}
\end{align}

Di sisi lain, bilangan kuantum topologi (\textit{Chern Number}, $C$) didefinisikan sebagai:
\begin{align}
    C &= \frac{1}{2\pi} \int_{\text{BZ}} \Omega(\mathbf{k}) \, d^2k 
\end{align}
Pada material fase topologis non-trivial ($C \neq 0$):
\begin{align}
    \frac{1}{2\pi} \int_{\text{BZ}} \Omega(\mathbf{k}) \, d^2k &\neq 0 \nonumber \\
    \oint_{\partial(\text{BZ})} \mathbf{A}(\mathbf{k}) \cdot d\mathbf{l} &\neq 0 \label{eq:chern_nonzero}
\end{align}

Persamaan (\ref{eq:stokes_zero}) dan (\ref{eq:chern_nonzero}) saling berkontradiksi. Hal ini membuktikan bahwa pada sistem dengan $C \neq 0$, Koneksi Berry $\mathbf{A}(\mathbf{k})$ dan fungsi Bloch $|u_{n\mathbf{k}}\rangle$ tidak mungkin terdefinisi secara \textit{smooth} di seluruh Hilbert manifold. Akibatnya, transformasi Fourier tidak konvergen secara eksponensial, dan representasi Wannier 3D yang terlokalisasi tidak dapat dikonstruksi.
